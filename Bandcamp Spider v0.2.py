...
#Functions
def validateInput(inputVar, inputType):
    if inputType == 0:
        ...
        if directoryValidation == False:
            ...
            return True
    #Checking if albumURL is a bandcamp URL
    #elif inputType == 1:
...
validationConiditonAlbumURL = False
albumURL = input("Enter target album URL: ")
validationConditionDownloadDestination = False
#get download destination file address (where track will be downloaded to
while validationConditionDownloadDestination == False:
    ...
    if downloadConfirm == "CONFIRM":
        validationConditionDownloadDestination = validateInput(albumURL, 0)
    
        
#Scrape album page**
print("Getting album page HTML...")
taskTimeStart = time.time()
albumSoup = createSoup(albumURL)
...
taskTimeStart = time.time()
coverTag = albumSoup.find("img", itemprop="image") #get album cover URL
...
tracks = albumSoup.find_all("a", href=re.compile("/track/"),itemprop="url")
...
tracksOnAlbum = albumSoup.find_all("span", itemprop={"name"}, text=True)
#print(tracksOnAlbum)
...
for i in range(0, trackLimit): #####replace with trackLimit for normal operation or 1 for testing
    ...
    trackSoup = createSoup(artistURL + trackList[i])
